...
const STYLE_BLOCK = `  performancePanel: { backgroundColor: "white", marginHorizontal: 14, marginTop: 12, borderRadius: 18, padding: 14, shadowColor: "#166534", shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.07, shadowRadius: 8, elevation: 2 },
...
function cleanupDuplicateStyles(text) {
  let next = text;
...
function repairTypeScriptIssues(text) {
  ...
  next = next.replace(/\n\s*\.\.\.\(\(?user(?: as any)?\)?\?\.dob \? \[\{ icon: "calendar" as const, label: "Date of Birth", value: String\([^\n]+?\) \}\] : \[\]\),/g, '');
  ...
  return cleanupDuplicateStyles(next);
}
...
try {
  ...
  if (!next.includes('Ward Performance')) {
    next = replaceOnce(
      next,
      '      {/* ALERTS PANEL */}',
  ...
  if (!next.includes('Search by complaint ID')) {
    next = replaceOnce(
      next,
  ...
  if (next !== text) {
    ...
    console.log('[Connect-T] Nagarsevak dashboard feature patch already clean');
  }
} catch (err) {
