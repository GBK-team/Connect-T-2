...
function DetailRow({ icon, label, value }: { icon: any; label: string; value?: string | number | null }) {
  ...
  return <View style={s.infoRow}><View style={s.infoIcon}><Feather name={icon} size={14} color={ORANGE} /></View><View style={{ flex: 1, minWidth: 0 }}><Text style={s.infoLabel}>{label}</Text><Text style={s.infoValue}>{value}</Text></View></View>;
}
...
function AppNotice({ visible, title, message, tone = "info", onClose }: { visible: boolean; title: string; message: string; tone?: NoticeTone; onClose: () => void }) {
  const color = tone === "success" ? "#16A34A" : tone === "danger" ? "#DC2626" : ORANGE;
  const bg = tone === "success" ? "#DCFCE7" : tone === "danger" ? "#FEE2E2" : "#FFF7ED";
  const icon = tone === "success" ? "check-circle" : tone === "danger" ? "alert-circle" : "info";
  return <Modal visible={visible} transparent animationType="fade" onRequestClose={onClose}><View style={s.noticeOverlay}><View style={s.noticeCard}><View style={[s.noticeIcon, { backgroundColor: bg }]}><Feather name={icon as any} size={26} color={color} /></View><Text style={s.noticeTitle}>{title}</Text><Text style={s.noticeMsg}>{message}</Text><TouchableOpacity style={[s.noticeOk, { backgroundColor: color }]} onPress={onClose}><Text style={s.noticeOkText}>OK</Text></TouchableOpacity></View></View></Modal>;
}
...
const s = StyleSheet.create({
  ...
  heroRow: { flexDirection: "row", alignItems: "center", gap: 13, marginTop: 18 },
  notFoundHero: { alignItems: "center", paddingTop: 20 },
  heroIcon: { width: 66, height: 66, borderRadius: 22, backgroundColor: "white", alignItems: "center", justifyContent: "center", shadowColor: DARK, shadowOpacity: 0.16, shadowRadius: 12, shadowOffset: { width: 0, height: 5 }, elevation: 7 },
  headerPill: { alignSelf: "flex-start", borderRadius: 999, backgroundColor: "rgba(255,255,255,0.14)", borderWidth: 1, borderColor: "rgba(255,255,255,0.16)", paddingHorizontal: 9, paddingVertical: 5, marginBottom: 6 },
  ...
  summaryCard: { marginTop: 16, backgroundColor: "rgba(255,255,255,0.15)", borderRadius: 17, padding: 12, flexDirection: "row", alignItems: "center", gap: 12, borderWidth: 1, borderColor: "rgba(255,255,255,0.12)" },
  summaryNumber: { fontSize: 24, color: "white", fontFamily: "Inter_700Bold", fontWeight: "900" },
  summaryLabel: { fontSize: 9.5, color: "rgba(255,255,255,0.75)", fontFamily: "Inter_400Regular" },
  summaryDivider: { width: 1, height: 36, backgroundColor: "rgba(255,255,255,0.18)" },
  summarySalary: { fontSize: 15, color: "white", fontFamily: "Inter_700Bold", fontWeight: "900" },
  summaryText: { fontSize: 10.5, color: "rgba(255,255,255,0.72)", fontFamily: "Inter_400Regular", marginTop: 1 },
  content: { padding: 16, gap: 12 },
  card: { backgroundColor: "white", borderRadius: 18, padding: 14, gap: 11, shadowColor: DARK, shadowOpacity: 0.05, shadowRadius: 10, shadowOffset: { width: 0, height: 4 }, elevation: 3, borderWidth: 1, borderColor: "rgba(254,215,170,0.9)" },
  metaRow: { flexDirection: "row", flexWrap: "wrap", gap: 7 },
  metaPill: { flexDirection: "row", alignItems: "center", gap: 5, backgroundColor: "#FFF7ED", borderRadius: 999, paddingHorizontal: 9, paddingVertical: 6, borderWidth: 1, borderColor: "#FED7AA" },
  metaPillText: { fontSize: 10.5, color: ORANGE, fontFamily: "Inter_700Bold" },
  sectionTitle: { fontSize: 14, fontWeight: "900", color: "#0F172A", fontFamily: "Inter_700Bold" },
  body: { fontSize: 12, color: "#334155", fontFamily: "Inter_400Regular", lineHeight: 18 },
  infoRow: { flexDirection: "row", alignItems: "flex-start", gap: 10, paddingVertical: 7, borderBottomWidth: 1, borderBottomColor: "#F8FAFC" },
  infoIcon: { width: 32, height: 32, borderRadius: 12, backgroundColor: "#FFF7ED", borderWidth: 1, borderColor: "#FED7AA", alignItems: "center", justifyContent: "center" },
  infoLabel: { fontSize: 10.5, color: "#94A3B8", fontFamily: "Inter_400Regular" },
  infoValue: { fontSize: 12.5, color: "#0F172A", fontFamily: "Inter_700Bold", marginTop: 1, lineHeight: 17 },
  actionRow: { flexDirection: "row", flexWrap: "wrap", gap: 9, marginTop: 4 },
  actionBtn: { flexGrow: 1, minWidth: 96, flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 7, backgroundColor: ORANGE, borderRadius: 15, paddingVertical: 12, paddingHorizontal: 12 },
  whatsappBtn: { flexGrow: 1, minWidth: 104, flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 7, backgroundColor: "#16A34A", borderRadius: 15, paddingVertical: 12, paddingHorizontal: 12 },
  resumeBtn: { flexGrow: 1, minWidth: 96, flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 7, backgroundColor: "#FFF7ED", borderWidth: 1, borderColor: "#FED7AA", borderRadius: 15, paddingVertical: 12, paddingHorizontal: 12 },
  resumeText: { color: ORANGE, fontSize: 12.5, fontWeight: "900", fontFamily: "Inter_700Bold" },
  appliedBtn: { backgroundColor: "#FFF7ED", borderWidth: 1, borderColor: "#FED7AA" },
  actionText: { color: "white", fontSize: 12.5, fontWeight: "900", fontFamily: "Inter_700Bold" },
  disabledBtn: { opacity: 0.45 },
  noteText: { fontSize: 10.5, color: "#94A3B8", fontFamily: "Inter_400Regular", lineHeight: 15 },
  emptyCard: { margin: 16, backgroundColor: "white", borderRadius: 20, padding: 24, alignItems: "center", gap: 10, shadowColor: DARK, shadowOpacity: 0.06, shadowRadius: 11, shadowOffset: { width: 0, height: 4 }, elevation: 3, borderWidth: 1, borderColor: "#FED7AA" },
  emptyTitle: { fontSize: 17, color: "#0F172A", fontFamily: "Inter_700Bold", fontWeight: "900" },
  emptyText: { fontSize: 11.5, color: "#64748B", fontFamily: "Inter_400Regular", textAlign: "center", lineHeight: 17 },
  emptyBtn: { marginTop: 4, backgroundColor: ORANGE, borderRadius: 999, paddingHorizontal: 17, paddingVertical: 9 },
  emptyBtnText: { color: "white", fontSize: 12.5, fontFamily: "Inter_700Bold", fontWeight: "900" },
