...
function parseSalaryMin(job: Job): number { if (typeof job.salaryMin === "number") return job.salaryMin; const matches = String(job.salary || "").match(/[\d,]+/g); return matches?.length ? Number(matches[0].replace(/,/g, "")) || 0 : 0; }
function parseSalaryMax(job: Job): number { if (typeof job.salaryMax === "number") return job.salaryMax; const matches = String(job.salary || "").match(/[\d,]+/g); if (!matches?.length) return parseSalaryMin(job); return Number(matches[matches.length - 1].replace(/,/g, "")) || parseSalaryMin(job); }
function timeAgo(dateStr: string) { const diff = Date.now() - new Date(dateStr).getTime(); const days = Math.floor(diff / 86400000); const hours = Math.floor(diff / 3600000); if (days > 0) return `${days}d ago`; if (hours > 0) return `${hours}h ago`; return "Just now"; }
function goBack(router: any) { if (router.canGoBack?.()) router.back(); else router.replace("/jobs/(tabs)" as any); }
function hasAreaMatch(jobLocation: string, area: string) { const loc = jobLocation.toLowerCase(); const target = area.toLowerCase(); if (loc.includes(target) || target.includes(loc)) return true; return target.split(/[\s,]+/).some((part) => part.length > 3 && loc.includes(part)); }
function isNearUser(job: Job, userLocation?: string) { if (!userLocation) return true; return hasAreaMatch(job.location, userLocation); }
function FilterChip({ label, active, onPress }: { label: string; active: boolean; onPress: () => void }) { return <TouchableOpacity style={[s.chip, active && s.chipActive]} onPress={onPress} activeOpacity={0.82}><Text style={[s.chipText, active && s.chipTextActive]}>{label}</Text></TouchableOpacity>; }
function getIcon(job: Job) { const cfg: any = categoryConfig[job.category]; return cfg?.icon || "briefcase"; }
function getType(job: Job) { const cfg: any = typeConfig[job.type]; return cfg || { label: job.type || "Job", bg: "#FFF7ED", color: ORANGE }; }

function AppNotice({ visible, title, message, onClose }: { visible: boolean; title: string; message: string; onClose: () => void }) {
  return <Modal visible={visible} transparent animationType="fade" onRequestClose={onClose}><View style={s.noticeOverlay}><View style={s.noticeCard}><View style={s.noticeIcon}><Feather name="briefcase" size={25} color={ORANGE} /></View><Text style={s.noticeTitle}>{title}</Text><Text style={s.noticeMsg}>{message}</Text><TouchableOpacity style={s.noticeOk} onPress={onClose} activeOpacity={0.85}><Text style={s.noticeOkText}>OK</Text></TouchableOpacity></View></View></Modal>;
}

function ResultCard({ job, applied, near, onApply, onOpen }: { job: Job; applied: boolean; near: boolean; onApply: () => void; onOpen: () => void }) {
  ...
  return (
    <TouchableOpacity style={s.card} onPress={onOpen} activeOpacity={0.9}>
      <View style={s.cardTop}>
        ...
        {near ? <View style={s.nearPill}><Feather name="map-pin" size={10} color={ORANGE} /><Text style={s.nearText}>Near</Text></View> : <Text style={s.cardTime}>{timeAgo(job.createdAt)}</Text>}
      </View>
      ...
      <View style={s.salaryRow}>
        <View style={{ flex: 1 }}><Text style={s.salary}>{job.salary}</Text><Text style={s.appliedCount}>{job.applicants.length} applicants · {timeAgo(job.createdAt)}</Text></View>
        <TouchableOpacity style={[s.applyBtn, applied && s.applyBtnDone]} onPress={(event) => { event.stopPropagation(); onApply(); }} disabled={applied} activeOpacity={0.86}>{applied ? <><Feather name="check" size={13} color={ORANGE} /><Text style={s.applyText}>Applied</Text></> : <Text style={s.applyTextWhite}>Apply Now</Text>}</TouchableOpacity>
      </View>
...
export default function SearchScreen() {
  ...
  const hasManualFilters = query || salaryMin || salaryMax || jobType !== "all" || category !== "all" || customCategory || selectedAreas.length > 0;
  ...
  const results = useMemo(() => jobs.filter((job) => {
    ...
    if (customCategoryText) { const customSearch = `${job.title} ${job.category} ${job.skillsRequired || ""}`.toLowerCase(); if (!customSearch.includes(customCategoryText)) return false; }
    else if (category !== "all" && job.category !== category) return false;
    if (jobType !== "all" && job.type !== jobType) return false;
    if (selectedAreas.length && !selectedAreas.some((area) => hasAreaMatch(job.location, area))) return false;
    if (!selectedAreas.length && !hasManualFilters && jobsUser?.location && !isNearUser(job, jobsUser.location)) return false;
    const low = parseSalaryMin(job);
    const high = parseSalaryMax(job);
    if (high < minSalary || low > maxSalary) return false;
    return true;
  }), [jobs, query, minSalary, maxSalary, jobType, category, customCategoryText, selectedAreas, hasManualFilters, jobsUser?.location]);
  ...
  return (
    <View style={s.root}>
      <LinearGradient colors={[DARK, ORANGE, "#F97316", "#FB923C"]} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={[s.header, { paddingTop: topPad + 10 }]}> 
        ...
        <View style={s.headerRow}><TouchableOpacity onPress={() => goBack(router)} style={s.backBtn} activeOpacity={0.84}><Feather name="arrow-left" size={20} color="white" /></TouchableOpacity><View style={{ flex: 1, minWidth: 0 }}><Text style={s.headerTitle}>Nearby Jobs</Text><Text style={s.headerSub}>Verified local openings with filters</Text></View>{hasManualFilters ? <TouchableOpacity onPress={clearAll} style={s.clearBtn}><Text style={s.clearBtnText}>Clear</Text></TouchableOpacity> : <View style={{ width: 58 }} />}</View>
        ...
        <View style={s.resultCount}><Text style={s.resultCountText}>{results.length} nearby jobs found</Text></View>
      </LinearGradient>
      <AppScrollView style={{ flex: 1 }} contentContainerStyle={[s.content, { paddingBottom: Math.max(insets.bottom, 8) + 28 }]} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
        <View style={s.filterCard}>
          <View style={s.filterBlock}><Text style={s.filterTitle}>Job Type</Text><AppScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={s.filterRow}>{JOB_TYPES.map((item) => <FilterChip key={item.id} label={item.label} active={jobType === item.id} onPress={() => setJobType(item.id)} />)}</AppScrollView></View>
          <View style={s.filterBlock}><Text style={s.filterTitle}>Category</Text><AppScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={s.filterRow}>{CATEGORIES.map((item) => <FilterChip key={item.id} label={item.label} active={category === item.id && !customCategory} onPress={() => { setCategory(item.id); setCustomCategory(""); }} />)}</AppScrollView><TextInput style={s.manualInput} placeholder="Or type custom skill/category…" placeholderTextColor="#94A3B8" value={customCategory} onChangeText={(value) => { setCustomCategory(value); if (value.trim()) setCategory("all"); }} /></View>
...
const s = StyleSheet.create({
  ...
  nearPill: { flexDirection: "row", alignItems: "center", gap: 4, backgroundColor: "#FFF7ED", borderRadius: 999, paddingHorizontal: 8, paddingVertical: 4, borderWidth: 1, borderColor: "#FED7AA" }, nearText: { fontSize: 9.5, color: ORANGE, fontFamily: "Inter_700Bold" },
  ...
  noticeOverlay: { flex: 1, backgroundColor: "rgba(15,23,42,0.45)", alignItems: "center", justifyContent: "center", padding: 22 }, noticeCard: { width: "100%", maxWidth: 340, backgroundColor: "white", borderRadius: 24, padding: 22, alignItems: "center", shadowColor: "#000", shadowOffset: { width: 0, height: 8 }, shadowOpacity: 0.15, shadowRadius: 24, elevation: 12 }, noticeIcon: { width: 54, height: 54, borderRadius: 20, alignItems: "center", justifyContent: "center", backgroundColor: "#FFF7ED", marginBottom: 12 }, noticeTitle: { fontSize: 17, color: "#0F172A", fontFamily: "Inter_700Bold", fontWeight: "900", textAlign: "center" }, noticeMsg: { marginTop: 6, fontSize: 13, color: "#64748B", fontFamily: "Inter_400Regular", lineHeight: 19, textAlign: "center" }, noticeOk: { marginTop: 18, borderRadius: 14, paddingVertical: 13, paddingHorizontal: 28, alignItems: "center", backgroundColor: ORANGE }, noticeOkText: { fontSize: 13, color: "white", fontFamily: "Inter_700Bold" },
});
