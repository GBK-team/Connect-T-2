...
function AppNotice({ visible, title, message, tone = "info", onClose, onDone }: { visible: boolean; title: string; message: string; tone?: "info" | "success" | "danger"; onClose: () => void; onDone?: () => void }) {
  const color = tone === "danger" ? "#DC2626" : ORANGE;
  const icon = tone === "success" ? "check-circle" : tone === "danger" ? "alert-circle" : "info";
  return <Modal visible={visible} transparent animationType="fade" onRequestClose={onClose}><View style={styles.noticeOverlay}><View style={styles.noticeCard}><View style={[styles.noticeIcon, { backgroundColor: tone === "danger" ? "#FEF2F2" : "#FFF7ED" }]}><Feather name={icon as any} size={26} color={color} /></View><Text style={styles.noticeTitle}>{title}</Text><Text style={styles.noticeMsg}>{message}</Text><TouchableOpacity style={[styles.noticeOk, { backgroundColor: color }]} onPress={onDone || onClose}><Text style={styles.noticeOkText}>OK</Text></TouchableOpacity></View></View></Modal>;
}
...
export default function EmployerPostJobScreen() {
  ...
  const [notice, setNotice] = useState({ visible: false, title: "", message: "", tone: "info" as "info" | "success" | "danger", done: undefined as undefined | (() => void) });
  ...
  const showNotice = (titleText: string, messageText: string, tone: "info" | "success" | "danger" = "info", done?: () => void) => setNotice({ visible: true, title: titleText, message: messageText, tone, done });
  ...
  if (!jobsUser || jobsUser.role !== "employer") return <View style={styles.emptyRoot}><View style={styles.emptyIcon}><Feather name="lock" size={32} color={ORANGE} /></View><Text style={styles.emptyTitle}>Employer access required</Text><Text style={styles.emptyText}>Please login as an employer to post jobs.</Text></View>;
  ...
  return (
    <View style={styles.root}>
      ...
      <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === "ios" ? "padding" : "height"} keyboardVerticalOffset={0}>
        <AppScrollView contentContainerStyle={[styles.content, { paddingBottom: insets.bottom + 160 }]} keyboardShouldPersistTaps="handled" keyboardDismissMode={Platform.OS === "ios" ? "interactive" : "on-drag"} automaticallyAdjustKeyboardInsets showsVerticalScrollIndicator={false}>
      ...
      <AppNotice visible={notice.visible} title={notice.title} message={notice.message} tone={notice.tone} onClose={() => setNotice((prev) => ({ ...prev, visible: false }))} onDone={notice.done} />
    </View>
  );
}
...
const styles = StyleSheet.create({
  ...
  urgentBox: { minHeight: 48, borderRadius: 14, backgroundColor: "#FFF7ED", borderWidth: 1, borderColor: "#FED7AA", flexDirection: "row", alignItems: "center", gap: 8, paddingHorizontal: 13 },
  urgentBoxActive: { backgroundColor: ORANGE, borderColor: ORANGE },
  urgentText: { color: ORANGE, fontSize: 12.5, fontFamily: "Inter_700Bold" },
  urgentTextActive: { color: "white" },
  previewText: { fontSize: 11.5, color: ORANGE, fontFamily: "Inter_700Bold", backgroundColor: "#FFF7ED", paddingHorizontal: 11, paddingVertical: 8, borderRadius: 13, borderWidth: 1, borderColor: "#FED7AA" },
  submitBtn: { height: 54, borderRadius: 17, backgroundColor: ORANGE, alignItems: "center", justifyContent: "center", flexDirection: "row", gap: 9, shadowColor: DARK, shadowOpacity: 0.18, shadowRadius: 10, shadowOffset: { width: 0, height: 5 }, elevation: 4 },
  submitBtnDisabled: { opacity: 0.65 },
  submitText: { color: "white", fontSize: 15, fontFamily: "Inter_700Bold", fontWeight: "900" },
  emptyRoot: { flex: 1, backgroundColor: BG, alignItems: "center", justifyContent: "center", padding: 24 },
  emptyIcon: { width: 70, height: 70, borderRadius: 24, backgroundColor: "#FFF7ED", borderWidth: 1, borderColor: "#FED7AA", alignItems: "center", justifyContent: "center" },
  emptyTitle: { marginTop: 14, fontSize: 18, color: "#0F172A", fontFamily: "Inter_700Bold", fontWeight: "900" },
  emptyText: { marginTop: 6, textAlign: "center", color: "#64748B", fontSize: 12, fontFamily: "Inter_600SemiBold" },
