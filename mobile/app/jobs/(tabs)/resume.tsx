...
export default function ResumeTab() {
  ...
  return (
    <View style={s.root}>
      <LinearGradient colors={[DARK, ORANGE, "#F97316", "#FB923C"]} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={[s.header, { paddingTop: topPad + 12 }]}> 
        ...
        <View style={s.headerRow}>
          <View style={{ flex: 1 }}>
            ...
            <Text style={s.headerSub}>{isEmployer ? "Maintain a trusted employer profile for applicants" : isReady ? "Your resume is ready to preview" : `${completion}% complete — resume can still be previewed`}</Text>
          </View>
      ...
      <AppScrollView style={{ flex: 1 }} contentContainerStyle={[s.content, { paddingBottom: Math.max(insets.bottom, 8) + 90 }]} showsVerticalScrollIndicator={false}>
        {isEmployer ? <View style={s.employerCard}><View style={s.employerIcon}><Feather name="shield" size={24} color={ORANGE} /></View><Text style={s.employerTitle}>Build applicant trust</Text><Text style={s.employerText}>Keep your company name, address, WhatsApp number, industry and description complete so job seekers can confidently apply.</Text><TouchableOpacity style={s.outlineBtn} onPress={() => router.push("/jobs/(tabs)/profile" as any)} activeOpacity={0.85}><Feather name="edit-3" size={16} color={ORANGE} /><Text style={s.outlineBtnText}>Edit Company Profile</Text></TouchableOpacity></View> : <TouchableOpacity style={s.actionCard} onPress={() => router.push("/jobs/resume" as any)} activeOpacity={0.85}><LinearGradient colors={[DARK, ORANGE, "#F97316"]} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={s.actionCardGrad}><View style={s.actionCardIcon}><Feather name="download" size={22} color={ORANGE} /></View><View style={{ flex: 1 }}><Text style={s.actionCardTitle}>{isReady ? "Resume Ready" : "Open Resume Builder"}</Text><Text style={s.actionCardSub}>{isReady ? "Preview and use your Connect T resume" : "Preview now and fill missing profile details anytime"}</Text></View><Feather name="chevron-right" size={20} color="white" /></LinearGradient></TouchableOpacity>}
