...
function initials(name?: string) { return String(name || "CT").split(" ").map((part) => part[0]).join("").slice(0, 2).toUpperCase(); }
function countWords(value?: string) { return String(value || "").trim().split(/\s+/).filter(Boolean).length; }
...
function Input(props: React.ComponentProps<typeof TextInput> & { label: string; helper?: string }) {
  ...
  return <View style={s.inputGroup}><Text style={s.label}>{label}</Text><TextInput {...rest} multiline={multiline} style={[s.input, multiline && s.textArea, style]} placeholderTextColor="#94A3B8" />{!!helper && <Text style={s.helper}>{helper}</Text>}</View>;
}
...
export default function JobPortalProfileScreen() {
  ...
  const pickPhoto = async () => {
    ...
    if (!permission.granted) { Alert.alert("Permission needed", "Please allow gallery access to update your photo."); return; }
  ...
  const saveProfile = async () => {
    if (saving) return;
    if (name.trim().length < 3) { Alert.alert("Check profile", "Enter a valid full name."); return; }
    if (!validEmail(email)) { Alert.alert("Check profile", "Enter a valid email address."); return; }
    if (!isEmployer && about.trim() && (countWords(about) < 5 || countWords(about) > 80)) { Alert.alert("Check profile", "About / Objective must be 5 to 80 words."); return; }
    if (isEmployer && companyDescription.trim() && countWords(companyDescription) > 100) { Alert.alert("Check profile", "Company description must be maximum 100 words."); return; }
    setSaving(true);
    try {
      if (isEmployer) {
    ...
    } catch (err: any) { Alert.alert("Save failed", err?.message || "Please try again."); }
