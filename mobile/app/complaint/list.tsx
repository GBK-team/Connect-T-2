...
const statusMeta: Record<
  ComplaintStatus,
  { title: string; subtitle: string; icon: string; color: string; bg: string }
> = {
  submitted: {
    title: "New Complaints",
    subtitle: "All newly submitted complaints",
    icon: "file-text",
    color: "#C2410C",
    bg: "#FFEDD5",
  },
  assigned: {
    ...
    subtitle: "Complaints assigned to ward team",
    icon: "user-check",
    color: "#EA580C",
    bg: "#FFEDD5",
  },
  in_progress: {
    ...
    subtitle: "Assigned and active complaints",
    icon: "tool",
    color: "#7C3AED",
    bg: "#EDE9FE",
  },
  resolved: {
    ...
    subtitle: "Complaints marked as resolved",
    icon: "check-circle",
    color: "#059669",
    bg: "#D1FAE5",
  },
...
function ComplaintRow({ complaint }: { complaint: ComplaintItem }) {
  const meta = statusMeta[complaint.status] || statusMeta.submitted;
  ...
  return (
    ...
    >
      <View style={[styles.rowIcon, { backgroundColor: meta.bg }]}>
        <Feather
          name={(categoryIcon[complaint.category] || "file-text") as any}
          size={19}
          color={meta.color}
        />
      </View>
      ...
      <View style={styles.rowBody}>
        ...
        <View style={styles.metaRow}>
          ...
          <View style={styles.metaChip}>
            ...
            <Text style={styles.metaText} numberOfLines={1}>
              {complaint.ward}
              {complaint.wardCode ? ` (${complaint.wardCode})` : ""}
            </Text>
          </View>
        ...
        <View style={styles.citizenRow}>
          ...
          {!!complaint.userMobile && (
            <Text style={styles.citizenText}>· {complaint.userMobile}</Text>
          )}
        </View>
      </View>
...
export default function ComplaintListScreen() {
  ...
  const selectedStatus =
    params.status && statusMeta[params.status] ? params.status : "submitted";
          ...
          ListEmptyComponent={
            <View style={styles.emptyCard}>
              ...
              <Text style={styles.emptyTitle}>
                No {pageMeta.title.toLowerCase()}
              </Text>
              <Text style={styles.emptySub}>
                Complaints for this status will appear here.
              </Text>
            </View>
          }
        />
