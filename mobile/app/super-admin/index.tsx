...
const ComplaintRow = memo(function ComplaintRow({
  ...
  return (
    ...
    >
      ...
      <View
        style={{
          backgroundColor: STATUS_BG[c.status] || "#F1F5F9",
          paddingHorizontal: 7,
          paddingVertical: 3,
          borderRadius: 8,
        }}
    ...
    >
      ...
      >
        ...
        {[
          ...
          {
            label: "Age",
            value: c.userAge ? String(c.userAge) : "—",
            icon: "calendar",
          },
            ...
            <View style={{ alignItems: "center", marginRight: 12 }}>
              <View
                style={{
                  ...
                  height: 10,
                  borderRadius: 5,
                  backgroundColor: STATUS_COLOR[t.status] || "#64748B",
                  marginTop: 3,
                }}
              />
...
export default function SuperAdminDashboard() {
  ...
  const stats = useMemo(() => {
    ...
    const wardOfficers = NAGARSEVAK_DIRECTORY.filter((n) =>
      n.ward.startsWith("Ward"),
    );
    ...
    () =>
      Array.from({ length: 6 }, (_, i) => {
        ...
        return {
          ...
          count: complaints.filter((c) => c.createdAt.startsWith(monthKey))
            .length,
        };
      }),
    [complaints],
  ...
  const wardOfficers = useMemo(
    () => NAGARSEVAK_DIRECTORY.filter((n) => n.ward.startsWith("Ward")),
    [],
  );
    ...
    (type: CardType) => {
      ...
      switch (type) {
        case "total":
          return sorted;
        case "pending":
          return sorted.filter((c) => c.status === "submitted");
        case "inProgress":
          return sorted.filter(
            (c) => c.status === "in_progress" || c.status === "assigned",
          );
        case "resolved":
          return sorted.filter((c) => c.status === "resolved");
        case "rejected":
      ...
      >
        ...
        >
          <StatCard
            ...
            bg="#DBEAFE"
            onPress={() =>
              openModal("total", "All Complaints", `${stats.total} complaints`)
            }
          />
          <StatCard
            icon="clock"
          ...
          <StatCard
            ...
            bg="#D1FAE5"
            onPress={() =>
              openModal("resolved", "Resolved", `${stats.resolved} completed`)
            }
          />
          <StatCard
            icon="x-circle"
            ...
            bg="#FEE2E2"
            onPress={() =>
              openModal("rejected", "Rejected", `${stats.rejected} rejected`)
            }
          />
          <StatCard
            icon="percent"
            ...
            bg="#E0F2FE"
            onPress={() =>
              openModal("resolution", "Resolution Rate", "Ward-wise analytics")
            }
          />
          <StatCard
            icon="users"
          ...
          <StatCard
            ...
            bg="#DCFCE7"
            onPress={() =>
              openModal("wards", "Ward Breakdown", "All wards overview")
            }
          />
        </View>
        ...
        <View style={{ marginTop: 16 }}>
          <SectionHeader
            ...
            sub="Complaint volume over 6 months"
          />
          <View
            style={{
              backgroundColor: "white",
                ...
                return (
                  <TouchableOpacity
                    ...
                    style={{
                      paddingVertical: 10,
                      borderBottomWidth:
                        idx < recentComplaints.length - 1 ? 1 : 0,
                      borderBottomColor: "#F1F5F9",
                    }}
                  >
                    <View
          ...
          <View style={{ flex: 1, backgroundColor: "#F0F4F8" }}>
            ...
            {selectedC ? (
              ...
              <FlatList
                ...
                renderItem={({ item: [ward, data], index }) => {
                  const d = data as any;
                  const officer = NAGARSEVAK_DIRECTORY.find(
                    (n) => n.ward === ward,
                  );
                  return (
                    <View
