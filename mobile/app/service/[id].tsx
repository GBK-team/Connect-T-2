...
function goBack() {
  ...
  else router.replace("/(tabs)/services" as any);
}
  ...
  if (!place || !category) {
    return (
      <View style={[styles.root, { justifyContent: "center", alignItems: "center" }]}> 
        ...
        <TouchableOpacity onPress={goBack} style={styles.notFoundBack} activeOpacity={0.85}>
          <Feather name="chevron-left" size={16} color="white" />
          <Text style={styles.notFoundBackText}>Back to Services</Text>
        </TouchableOpacity>
      </View>
    );
  }
  ...
  return (
    <View style={styles.root}>
      ...
      <ScrollView style={styles.scroll} contentContainerStyle={[styles.scrollContent, { paddingBottom: Math.max(insets.bottom, 8) + 24 }]} showsVerticalScrollIndicator={false}>
        ...
        <ServiceMap address={place.address} name={place.name} color={category.color} bgColor={category.bgColor} />
        ...
        {place.rating !== undefined && (
          <View style={styles.ratingSection}>
            <View style={styles.ratingLeft}>
              ...
              {place.reviewCount !== undefined && <Text style={styles.reviewCount}>{place.reviewCount} reviews</Text>}
            </View>
        ...
        {place.beds !== undefined && place.bedsOccupied !== undefined && (
          <View style={styles.card}>
            ...
            <View style={styles.bedStats}>
              <View style={styles.bedStat}><Text style={styles.bedNum}>{place.beds}</Text><Text style={styles.bedLabel}>Total Beds</Text></View>
              <View style={styles.bedStat}><Text style={[styles.bedNum, { color: "#DC2626" }]}>{place.bedsOccupied}</Text><Text style={styles.bedLabel}>Occupied</Text></View>
              <View style={styles.bedStat}><Text style={[styles.bedNum, { color: "#059669" }]}>{bedsAvailable}</Text><Text style={styles.bedLabel}>Available</Text></View>
            </View>
            <View style={styles.bedBarTrack}>
              <View style={[styles.bedBarFill, { width: `${bedsFillPct?.toFixed(0)}%` as any, backgroundColor: (bedsFillPct ?? 0) > 85 ? "#DC2626" : (bedsFillPct ?? 0) > 65 ? "#D97706" : "#059669" }]} />
            </View>
            <Text style={styles.bedBarLabel}>{bedsFillPct?.toFixed(0)}% occupancy{(bedsFillPct ?? 0) > 85 ? " — Nearly Full" : (bedsFillPct ?? 0) > 65 ? " — Moderately Full" : " — Beds Available"}</Text>
          </View>
        )}
        ...
        {place.services && place.services.length > 0 && (
          <View style={styles.card}>
            ...
            {place.services.map((svc, i) => <View key={i} style={styles.serviceRow}><View style={[styles.serviceDot, { backgroundColor: category.color }]} /><Text style={styles.serviceRowText}>{svc}</Text></View>)}
          </View>
        )}
        ...
        <View style={styles.card}>
          ...
          {place.contacts.map((contact, i) => (
            <TouchableOpacity key={i} style={[styles.contactRow, { borderColor: category.color + "25" }]} onPress={() => handleCall(contact.phone)} activeOpacity={0.75}>
              ...
              <View key={i} style={[styles.reviewRow, i < place.reviews!.length - 1 && styles.reviewRowBorder]}>
                <View style={styles.reviewTop}>
                  <View style={[styles.reviewAvatar, { backgroundColor: category.bgColor }]}> 
                    <Text style={[styles.reviewAvatarText, { color: category.color }]}>{review.reviewer.charAt(0).toUpperCase()}</Text>
                  </View>
                  <View style={styles.reviewMeta}>
                    <Text style={styles.reviewName}>{review.reviewer}</Text>
                    <View style={styles.reviewStarRow}><StarRating rating={review.rating} size={11} /><Text style={styles.reviewDate}>{review.date}</Text></View>
                  </View>
                </View>
...
const styles = StyleSheet.create({
  ...
  headerBack: { width: 38, height: 38, borderRadius: 12, backgroundColor: "rgba(255,255,255,0.2)", alignItems: "center", justifyContent: "center", marginBottom: 16 },
  headerContent: { alignItems: "center" },
  headerIconCircle: { width: 72, height: 72, borderRadius: 22, alignItems: "center", justifyContent: "center", marginBottom: 12 },
  headerName: { fontSize: 20, fontWeight: "900", color: "white", fontFamily: "Inter_700Bold", textAlign: "center", marginBottom: 10, letterSpacing: -0.3 },
  ...
  section: { backgroundColor: "white", borderRadius: 16, padding: 14, marginBottom: 12, shadowColor: "#B45309", shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.06, shadowRadius: 8, elevation: 2 },
  ...
  ratingSection: { backgroundColor: "white", borderRadius: 16, padding: 16, marginBottom: 12, flexDirection: "row", alignItems: "center", gap: 16, shadowColor: "#B45309", shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.06, shadowRadius: 8, elevation: 2 },
  ...
  card: { backgroundColor: "white", borderRadius: 16, padding: 16, marginBottom: 12, shadowColor: "#B45309", shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.06, shadowRadius: 8, elevation: 2 },
  ...
  contactRow: { flexDirection: "row", alignItems: "center", gap: 12, borderWidth: 1, borderRadius: 12, padding: 12, marginBottom: 8, backgroundColor: "#F8FAFC" },
  ...
  notFoundBack: { marginTop: 16, flexDirection: "row", alignItems: "center", gap: 6, backgroundColor: "#EA580C", borderRadius: 14, paddingHorizontal: 15, paddingVertical: 10 },
  notFoundBackText: { color: "white", fontSize: 13, fontFamily: "Inter_700Bold" },
});
