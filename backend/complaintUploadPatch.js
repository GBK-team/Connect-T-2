...
async function createMultipartComplaint(req, res) {
  ...
  try {
    ...
    const category = cleanText(req.body?.category || "other", 80) || "other";
    ...
    const [existingRows] = await pool.query(
      ...
      [requestId, user.id, userMobile],
    );
    if (existingRows.length) {
      return sendJson(res, 200, {
        success: true,
        duplicate: true,
        complaintId: existingRows[0].id,
        photo_url: existingRows[0].photo_url,
        ward_code: existingRows[0].ward_code,
        assigned_officer_id: existingRows[0].assigned_officer_id,
      });
    }
    ...
    if (!wardCode) {
      return sendJson(res, 400, { success: false, code: "WARD_REQUIRED", message: "Select a valid ward from Ward 1 to Ward 29." });
    }
    ...
    return sendJson(res, status, {
      ...
      message: status >= 500
        ? "The complaint image could not be uploaded right now. Please try again."
